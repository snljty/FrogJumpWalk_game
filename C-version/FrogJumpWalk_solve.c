...
bool isPrintWithColor = false;

void PrintSymbol(char const *operation, int index);
...
int main(int argc, char const *argv[])
{
    ...
    if (getenv("PRINT_WITH_COLOR"))
    {
        isPrintWithColor = true;
        system("");
    }
    else
    {
        isPrintWithColor = false;
    }
    ...
    for (i = 1; i <= nFrogs; i ++)
    {
        for (j = 1; j <= i; j ++)
        {
            if (j == i)
            {
                PrintSymbol(walk, ! (i % 2));
            }
            else
            {
                PrintSymbol(jump, ! (i % 2));
            }
        }
    }
    for (j = 1; j <= nFrogs; j ++)
    {
        PrintSymbol(jump, nFrogs % 2);
    }
    for (i = nFrogs; i >= 1; i --)
    {
        for (j = i; j >= 1; j --)
